...
const decreaseImages = filesToPhotosObject(
  require.context(
    "./../../../public/images/cognitive-reappraisal/decrease/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const DecreaseImageFileNameArray = Object.keys(decreaseImages);
const decreaseFullFileNameArray = DecreaseImageFileNameArray.map(
  (e) => "./public/images/cognitive-reappraisal/decrease/" + e
);
const randomizedDecreaseFileNameArray = decreaseFullFileNameArray.sort(
  () => 0.5 - Math.random()
);

const lookNegativeImages = filesToPhotosObject(
  require.context(
    "./../../../public/images/cognitive-reappraisal/look-negative/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const lookNegativeImagesFileNameArray = Object.keys(lookNegativeImages);
const lookNegativeFullFileNameArray = lookNegativeImagesFileNameArray.map(
  (e) => "./public/images/cognitive-reappraisal/look-negative/" + e
);
const randomizedLookNegativeFileNameArray = lookNegativeFullFileNameArray.sort(
  () => 0.5 - Math.random()
);

const lookNeutralImages = filesToPhotosObject(
  require.context(
    "./../../../public/images/cognitive-reappraisal/look-neutral/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const lookNeutralImagesFileNameArray = Object.keys(lookNeutralImages);
const lookNeutralImagesFullFileNameArray = lookNeutralImagesFileNameArray.map(
  (e) => "./public/images/cognitive-reappraisal/look-neutral/" + e
);
const randomizedlookNeutralFileNameArray =
  lookNeutralImagesFullFileNameArray.sort(() => 0.5 - Math.random());

const titleCardImages = filesToPhotosObject(
  require.context(
    "./../../../public/images/cognitive-reappraisal/titlecards/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const titleCardImagesFileNameArray = Object.keys(titleCardImages);
export const titleCardImagesFullFileNameArray =
  titleCardImagesFileNameArray.map(
    (e) => "./public/images/cognitive-reappraisal/titlecards/" + e
  );
...
const shuffleArrayAintoArrayB = (arrayA, arrayB) => {
  ...
  while (blockedArray.length < arrayA.length) {
    ...
    if (
      !blockedArray.includes(randomOddNumber)
...
const combinedNegAndNeutralCognitiveImagesArray =
  randomizedLookNegativeFileNameArray.concat(
    randomizedlookNeutralFileNameArray
  );
const randomizedNegAndNeutralCombinedCognitiveImagesArray =
  combinedNegAndNeutralCognitiveImagesArray.sort(() => 0.5 - Math.random());
const randomizedNegAndNeutralAndDecreaseCombinedCognitiveImagesArray =
  shuffleArrayAintoArrayB(
    randomizedDecreaseFileNameArray,
    randomizedNegAndNeutralCombinedCognitiveImagesArray
  );

const randomizedCombinedCognitiveImagesArrayWithTitleCards =
  addTitleCardsToCognitiveImagesArray(
    randomizedNegAndNeutralAndDecreaseCombinedCognitiveImagesArray
  );
export const finalRandomizedCombinedCognitiveImagesArray =
  randomizedCombinedCognitiveImagesArrayWithTitleCards.map(
    (x) => "./" + x.slice(8)
  );
