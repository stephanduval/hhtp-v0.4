...
import /*checkForEmptyinArray, showMatchesOnly */ "./functions";
import { filesToPhotosObject } from "../../functions.js";
import { useSelector, useDispatch /*, connect*/ } from "react-redux";
import {
  /*setCorrectResponseArray, setUserResponseArray,*/ setImageFileNameArray,
  setImageFileNameLength,
  setPredictiveImageFileNameArray,
  setPredictiveImageFileNameLength,
  setImageSet,
  setScoringArray,
  setFinalFileNameArray,
  setImageSetStageOne,
  setImageSetStageTwo,
  setImageSetStageThree,
  setCorrectScoresrray,
  setNBackIndex,
  setPredictiveIndex,
} from "./actions";
import { /*randomizeArray,*/ arrayLength } from "./../../functions";
import /*validateNumberOfPhotos, validateNumberOfPredictivePhotos, validatesetNumberofnBackMatches, validatesetnBackDegree, validatesetTimerSeconds*/ "./../../functions";
// ======== Constants:
//

//const images = filesToPhotosObject(require.context('./../../../public/images/n-back-photos/RandomLot/', false, /\.(png|jpe?g|svg)$/));
const randomImagesNegative = filesToPhotosObject(
  require.context(
    "./../../../public/images/n-back-photos/Remaining Pictures Negative (34 Total)/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const randomImagesNeutral = filesToPhotosObject(
  require.context(
    "./../../../public/images/n-back-photos/Remaining Pictures Neutral (34 Total)/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
...
const imageFileNamerandomImagesNegativeFullFileNameArray =
  imageFileNamerandomImagesNegative.map(
    (e) => "./images/n-back-photos/Remaining Pictures Negative (34 Total)/" + e
  );
const imageFileNamerandomImagesNeutralFullFileNameArray =
  imageFileNamerandomImagesNeutral.map(
    (e) => "./images/n-back-photos/Remaining Pictures Neutral (34 Total)/" + e
  );
let imageRemainingFileNameArray =
  imageFileNamerandomImagesNegativeFullFileNameArray.concat(
    imageFileNamerandomImagesNeutralFullFileNameArray
  );
//imageRemainingFileNameArray = ["a","b","c","d","e","f","g","h","i","j","empty","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z"];
let randomizedRemainingFileNameArray = imageRemainingFileNameArray.sort(
  () => 0.5 - Math.random()
);
...
const predictiveImagesNegative = filesToPhotosObject(
  require.context(
    "./../../../public/images/n-back-photos/Prospective Memory Hits (12 Total)/Negative Prospective Memory Hits (6 Total)/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const predictiveImagesNeutral = filesToPhotosObject(
  require.context(
    "./../../../public/images/n-back-photos/Prospective Memory Hits (12 Total)/Neutral Prospective Memory Hits (6 Total)/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
...
const nBackImagesNegative = filesToPhotosObject(
  require.context(
    "./../../../public/images/n-back-photos/Negative N-Back Hits (13 Total)/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const nBackImagesNeutral = filesToPhotosObject(
  require.context(
    "./../../../public/images/n-back-photos/Neutral N-Back Hits (13 Total)/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
...
const imageFileNamenBackImagesNegativeFullFileNameArray =
  imageFileNamenBackImagesNegative.map(
    (e) => "./images/n-back-photos/Negative N-Back Hits (13 Total)/" + e
  );
const imageFileNamenBackImagesNeutralFullFileNameArray =
  imageFileNamenBackImagesNeutral.map(
...
let predictiveImagesNegativeFileNameArray = Object.keys(
  predictiveImagesNegative
);
let predictiveImagesNeutralFileNameArray = Object.keys(predictiveImagesNeutral);
predictiveImagesNegativeFileNameArray =
  predictiveImagesNegativeFileNameArray.map(
    (e) =>
      "./images/n-back-photos/Prospective Memory Hits (12 Total)/Negative Prospective Memory Hits (6 Total)/" +
      e
  );
predictiveImagesNeutralFileNameArray = predictiveImagesNeutralFileNameArray.map(
  (e) =>
    "./images/n-back-photos/Prospective Memory Hits (12 Total)/Neutral Prospective Memory Hits (6 Total)/" +
    e
);
...
const PredictveImageArrayDispatch = (dispatch) => ({
  setPredictiveImageFileNameArray: (array) =>
    dispatch(setPredictiveImageFileNameArray(array)),
});
const PredictiveImageFileNameLengthDispatch = (dispatch) => ({
  setPredictiveImageFileNameLength: (array) =>
    dispatch(setPredictiveImageFileNameLength(array)),
});
...
const ImageArray = () => {
  ...
  const { setPredictiveImageFileNameArray } = PredictveImageArrayDispatch(
    useDispatch()
  ); // how does this work?  It creates an object
  const { setPredictiveImageFileNameLength } =
    PredictiveImageFileNameLengthDispatch(useDispatch());
  ...
  const ReduxStorefileNameArray = useSelector(
    (state) => state.imageArrayReducer.imageFileNameArray
  );
  ...
  const PredictiveFullFileNameArrayLength = arrayLength(
    predictiveFullFileNameArray
  );
  ...
  const numberOfPhotos = useSelector(
    (state) => state.nBackSettingsReducer.numberOfPhotos
  );
  const numberOfPredictivePhotos = useSelector(
    (state) => state.nBackSettingsReducer.numberOfPredictivePhotos
  );
  const NumberofnBackMatches = useSelector(
    (state) => state.nBackSettingsReducer.NumberofnBackMatches
  );
  const nBackDegree = useSelector(
    (state) => state.nBackSettingsReducer.nBackDegree
  );

  let masterImageSet = Array.apply("null", Array(numberOfPhotos)).map(
    function () {}
  );
  ...
  const predictiveIndexes = (masterImageSet, numberOfPredictivePhotos) => {
    ...
    while (indexOfPredictivePhotos.length < numberOfPredictivePhotos) {
      ...
      if (!indexOfPredictivePhotos.includes(r)) {
  ...
  const imageSetStageTwo = (
    ...
    predictive.forEach((element) =>
      masterImageSet.splice(
        predictiveIndex[iteratePredictiveIndexTwo()],
        1,
        PredictiveFullFileNameArray[iteratePredictiveReduxValue()]
      )
    );
    return masterImageSet; //.slice(0,numberOfPhotos);
  };

  const imageStageTwo = imageSetStageTwo(
    masterImageSet,
    predictiveIndex,
    predictiveFullFileNameArray
  );
  masterImageSet = imageSetStageTwo(
    masterImageSet,
    predictiveIndex,
    predictiveFullFileNameArray
  );
  ...
  const nBackIndexes = (
    ...
    while (nBackIndex.length < NumberofnBackMatches) {
      let num = Math.floor(
        Math.random() * (numberOfPhotos - nBackDegree) + nBackDegree
      );
  ...
  const createStageThreeSafeShuffleIndexFunction = (
    predictiveIndex,
    nBackIndex,
    nBackDegree,
    NumberofnBackMatches
    ...
    while (stageThreeSafeShuffleIndex.length < NumberofnBackMatches) {
      let num = Math.floor(
        Math.random() *
          (numberOfPhotos - NumberofnBackMatches - numberOfPredictivePhotos)
      );
  ...
  const stageThreeSafeShuffleIndexFunction = (
    inputArray,
    stageThreeSafeShuffleIndex
  ) => {
    stageThreeSafeShuffleIndex.forEach((element) =>
      inputArray.splice(element, 0, inputArray.pop())
    );
  ...
  const imageSetStageThree = (
    nBackIndex,
    nBackDegree,
    masterImageSet,
    nBackFullFileNameArray,
    ...
    nBackIndex.forEach((element) =>
      stage2.splice(
        element - nBackDegree,
        1,
        nBackFullFileNameArray[nBackIndex.indexOf(element)]
      )
    );
    nBackIndex.forEach((element) =>
      stage2.splice(
    ...
    randomizedEmptyIndex.forEach((element) =>
      stage2.splice(
        element,
        1,
        imageRemainingFileNameArray[randomizedEmptyIndex.indexOf(element)]
      )
    );
  ...
  let imageStageThreeSafeShuffleIndex =
    createStageThreeSafeShuffleIndexFunction(
      predictiveIndex,
      nBackIndex,
      nBackDegree,
      NumberofnBackMatches
  ...
  let imageStageThree = imageSetStageThree(
    nBackIndex,
    nBackDegree,
    imageStageTwo,
    nBackFullFileNameArray,
  ...
  let imageStageThreeForScoringArray = imageSetStageThree(
    nBackIndex,
    nBackDegree,
    imageStageTwo,
    nBackFullFileNameArray
  ...
  const scoringArray = (imageStageThree, predictiveIndex, nBackIndex) => {
    ...
    let scoreArray = Array.apply("null", Array(scoreLength)).map(
      function () {}
    );
    scoreArray.fill("-");

    predictive.forEach((element) =>
      scoreArray.splice(predictive[iteratePredictiveIndex()], 1, "P")
    );

    let nBackIndexValue = 0;
    const iteratenBackIndex = () => nBackIndexValue++;
    let nBackReduxValue = 0;
    nBack.forEach((element) =>
      scoreArray.splice(nBack[iteratenBackIndex()], 1, "N")
    );

    return scoreArray;
  };
  //imageStageThree = stageThreeSafeShuffleIndexFunction(imageStageThree,imageStageThreeSafeShuffleIndex);
  let correctScoresrray = scoringArray(
    imageStageThree,
    predictiveIndex,
    nBackIndex
  );
...
*/
  ...
  return <div className="imageArray"></div>;
};
