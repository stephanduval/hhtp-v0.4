...
import {
  setRenderState,
  newNBackState,
  newUserResponseArray,
  newUserAnswerTimeArray,
  duplicateTheImageArray,
} from "./actions";
...
const duplicateTheImageArrayDispatch = (dispatch) => ({
  duplicateTheImageArray: (duplicateImageArray) =>
    dispatch(duplicateTheImageArray(duplicateImageArray)),
});
...
const ExamNavigation = () => {
  ...
  const timerSeconds =
    1000 * useSelector((state) => state.nBackSettingsReducer.timerSeconds);
  const imageSetStageThreeFromRedux = useSelector(
    (state) => state.imageArrayReducer.imageSetStageThree
  );
  const { duplicateTheImageArray } = duplicateTheImageArrayDispatch(
    useDispatch()
  );
  ...
  console.log(
    "imageSetStageThreeFromRedux from navigation log",
    imageSetStageThreeFromRedux
  );
...
*/
  ...
  const keyStrokeListener = (event) => {
    ...
    switch (event.keyCode) {
      case 87:
        return (
          newNBackState(NBackState),
          addNBackToUserResponseArray(),
          clearInterval(switchPhotosOnInterval),
          addTimeToUserAnswerTimeArray(timeTakenToAnswer(timerStart)),
      ...
      case 79:
        return (
          newNBackState(NBackState),
          addPredictiveToUserResponseArray(),
          clearInterval(switchPhotosOnInterval),
          addTimeToUserAnswerTimeArray(timeTakenToAnswer(timerStart)),
          checkIfTestIsComplete()
        );
  ...
  return (
    <div className="buttonSpace">
      ...
      >
        "W" - Same as {nBackDegree} photos Back
      </Button>
