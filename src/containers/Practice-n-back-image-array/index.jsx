...
const practiceImages = filesToPhotosObject(
  require.context(
    "./../../../public/images/practice-n-back-photos/RemainingPictures/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const practiceImageFileNameArray = Object.keys(practiceImages);
const practiceImageFullFileNameArray = practiceImageFileNameArray.map(
  (e) => "./images/practice-n-back-photos/RemainingPictures/" + e
);
...
const practiceNBackImages = filesToPhotosObject(
  require.context(
    "./../../../public/images/practice-n-back-photos/Two-Back-Hits/",
    false,
    /\.(png|jpe?g|svg)$/
  )
);
const practiceNBackImageFileNameArray = Object.keys(practiceNBackImages);
const practiceNBackFullFileNameArray = practiceNBackImageFileNameArray.map(
  (e) => "./images/practice-n-back-photos/Two-Back-Hits/" + e
);

const numberOfPhotos =
  practiceImageFullFileNameArray.length +
  practiceNBackFullFileNameArray.length * 2;
...
const testPracticeStateArrayDispatch = (dispatch) => ({
  setTestPracticeStateArray: (array) =>
    dispatch(setTestPracticeStateArray(array)),
});
...
const PracticeImageArray = () => {
  ...
  const { setTestPracticeStateArray } = testPracticeStateArrayDispatch(
    useDispatch()
  );
  ...
  const imageSetStageOne = practiceImageFullFileNameArray.slice(
    0,
    numberOfPhotos
  ); // when number of photos is removed from here then it works
  const imageSetStageTwo = (
    imageSetStageOne,
    practiceNBackFullFileNameArray,
    numberOfPhotos
  ) => {
    /*
  ...
  let resultCheckerArray = [
    ...
    {
      name: "imageSetStageOne",
      arrayData: imageSetStageOne.map((element) => element.slice(-25, -4)),
    },
    {
      name: "imageStageTwo",
      arrayData: imageStageTwo.map((element) => element.slice(-21, -4)),
    },
  ];
