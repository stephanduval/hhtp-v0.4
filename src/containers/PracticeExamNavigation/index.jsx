...
const setPracticeRenderStateDispatch = (dispatch) => ({
  setPracticeRenderState: (nameState) =>
    dispatch(setPracticeRenderState(nameState)),
});
...
const PracticeExamNavigation = () => {
  ...
  const practiceImageArrayFromRedux = useSelector(
    (state) => state.practiceImageArrayReducer.practiceImageArray
  );

  const NBackState = useSelector(
    (state) => state.practiceExamNavigationReducer.newPracticeNBackState
  );
  const { newPracticeNBackState } = newPracticeNBackStateDispatch(
    useDispatch()
  );
  const { setPracticeRenderState } = setPracticeRenderStateDispatch(
    useDispatch()
  );
  ...
  const keyStrokeListener = (event) => {
    ...
    switch (event.keyCode) {
      case 87:
        return (
          (modalCheckResult = practiceCheckResult(
            "nBackButton",
            practiceArraySourceSlice
          )),
          setShow(true)
        );
      case 79:
  ...
  let practiceArraySourceSlice = practiceImageArrayFromRedux[NBackState].slice(
    32,
    -9
  );
  ...
  const practiceCheckResult = (buttonSwitchParam, imageSourceStringParam) => {
    switch (buttonSwitchParam) {
      case "nBackButton":
        if (
          imageSourceStringParam.includes("Two-Back-Hits", 0) &&
          practiceImageArrayFromRedux[NBackState] ==
            practiceImageArrayFromRedux[NBackState - 2]
        ) {
          return {
            modalTitle: "CORRECT!",
            modalMessage:
              "The image you are looking at appeared two images prior to this one. You're getting it",
          };
        }
        if (
          imageSourceStringParam.includes("Two-Back-Hits", 0) &&
          ...
          return {
            ...
            modalMessage:
              "You have not seen this image before.  However you may see it repeated later.",
          };
        }
        if (imageSourceStringParam.includes("RemainingPictures", 0)) {
          return {
            modalTitle: "Wrong Answer!",
            modalMessage:
              "The image you are looking at is a unique Image. You have not seen it before",
          };
        } else {
          return {
            modalTitle: "Wrong Answer!",
      ...
      case "predictiveButton":
        ...
        if (
          ...
          return {
            modalTitle: "Wrong Answer!",
            modalMessage:
              'This image appeared two images ago.  When you see an image repreat with one image in between click the "Same As 2 Photos Back" button',
          };
        }
        if (imageSourceStringParam.includes("RemainingPictures", 0)) {
          return {
            modalTitle: "Wrong Answer!",
            modalMessage:
              "This task will be introduced later. For now, please only identify whether you are seeing a unique or 2-back picture.",
          };
        } else {
          return {
            modalTitle: "Wrong Answer!",
      ...
      case "uniqueButton":
        if (imageSourceStringParam.includes("RemainingPictures", 0)) {
          return {
            modalTitle: "CORRECT!",
            modalMessage:
              "This is a unique image.  You have not seen it before.",
          };
        }
        if (
          imageSourceStringParam.includes("Two-Back-Hits", 0) &&
        ...
        if (
          ...
          return {
            modalTitle: "Wrong Answer!!",
            modalMessage:
              "The image you are looking at appeared two images prior to this one",
          };
        } else {
          return {
            modalTitle: "Wrong Answer!",
    ...
    <div className="container">
      <div className="buttonSpace">
        <Button
          color="primary"
          variant="contained"
          stringvalue={"Same as *n* photos Back"}
          onClick={() => {
            modalCheckResult = practiceCheckResult(
              "nBackButton",
              practiceArraySourceSlice
        ...
        >
          "W" - Same as 2 photos Back
        </Button>

        <Button
          color="default"
          variant="contained"
          stringvalue={"O - Predictive"}
          onClick={() => {
            modalCheckResult = practiceCheckResult(
              "predictiveButton",
              practiceArraySourceSlice
        ...
        >
          "O" - Predictive - I was told to remember this
        </Button>

        <Button
          color="secondary"
          variant="contained"
          stringvalue={"S - Unique Image"}
          onClick={() => {
            modalCheckResult = practiceCheckResult(
              "uniqueButton",
              practiceArraySourceSlice
      ...
      <div className="Modal">
        <GenericModal
          title={modalCheckResult.modalTitle}
          onClose={() => onCloseWrapperFunction()}
          show={show}
        >
