...
const CSVDownloadDiv = () => {
  ...
  const duplicatedImageArray = useSelector(
    (state) => state.examNavigationReducer.duplicateImageArray
  );
  ...
  const shortenedImageStageThree = imageSetStageThreeFromRedux.map(
    (element) => element
  );
  const shortenedcorrectScoresrray = correctScoresrray.map(
    (element) => element
  );
  ...
  const dateForCSVFileName =
    "_" +
    dateAndTimeOfCSVCreation.getFullYear() +
    "-" +
    dateAndTimeOfCSVCreation.getMonth() +
    "-" +
    dateAndTimeOfCSVCreation.getDate() +
    "_" +
    dateAndTimeOfCSVCreation.getHours() +
    "h" +
    dateAndTimeOfCSVCreation.getMinutes() +
    "m";
  ...
  return (
    <div>
      <CSVLink filename={CSVFileNameString} data={csvData}>
        Download CSV
      </CSVLink>
    </div>
  );
